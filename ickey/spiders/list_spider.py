...
class ListSpider:
    ...
    def crawl(
        ...
        for i, item in enumerate(all_items):
            ...
            try:
                ...
                for p in products:
                    append_to_json_file(p, output_path)
    ...
    def _refresh_tokens(self) -> None:
        """从列表首页获取 _csrf 和 v_"""
        resp = self.session.get(CATE_PAGE_URL, params={"cate_id": "010101"}, timeout=self.session.timeout)
        resp.raise_for_status()
        html = resp.text
        ...
        logger.debug(f"Tokens refreshed: v_={self._v_[:20]}..., _csrf={self._csrf[:20]}...")
